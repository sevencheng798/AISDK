...
bool extractPlaylistContent(std::unique_ptr<utils::HTTPContent> httpContent, std::string* content) {
    if (!content) {
        AISDK_ERROR(LX("getContentFromPlaylistUrlIntoStringFailed").d("reason", "nullString"));
        return false;
    }
    if (!httpContent) {
    ...
    if (!(*httpContent)) {
        AISDK_ERROR(LX("getContentFromPlaylistUrlIntoStringFailed").d("reason", "badHTTPContentReceived"));
        return false;
    }

#if 0
    auto reader = httpContent->dataStream->createReader(utils::sds::ReaderPolicy::BLOCKING);
    if (!reader) {
        AISDK_ERROR(LX("getContentFromPlaylistUrlIntoStringFailed").d("reason", "failedToCreateStreamReader"));
        return false;
    }
    ...
    while (!streamClosed) {
        ...
        switch (readStatus) {
            ...
            case avsCommon::avs::attachment::AttachmentReader::ReadStatus::ERROR_INTERNAL:
                AISDK_ERROR(LX("getContentFromPlaylistUrlIntoStringFailed").d("reason", "readError"));
                return false;
        }
    }
