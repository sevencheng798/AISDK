...
#include <Utils/PlaylistParser/PlaylistParserObserverInterface.h>
...
static const std::chrono::milliseconds INVALID_DURATION =
    utils::playlistParser::PlaylistParserObserverInterface::INVALID_DURATION;
...
std::unique_ptr<IterativePlaylistParser> IterativePlaylistParser::create(
    ...
    if (!contentFetcherFactory) {
        AISDK_ERROR(LX("createFailed").d("reason", "nullContentFetcherFactory"));
        return nullptr;
    }
...
bool IterativePlaylistParser::initializeParsing(std::string url) {
    if (url.empty()) {
        AISDK_ERROR(LX("parsePlaylistFailed").d("reason", "emptyUrl"));
        return false;
    }
...
IterativePlaylistParser::PlaylistEntry IterativePlaylistParser::next() {
    while (!m_urlsToParse.empty() && !m_abort) {
        ...
        if (urlAndInfo.length != INVALID_DURATION) {
            ...
            AISDK_DEBUG5(LX("foundMediaURL"));
            return PlaylistEntry{
                .url = urlAndInfo.url,
                .duration = urlAndInfo.length,
                .parseResult = m_urlsToParse.empty()
                                   ? utils::playlistParser::PlaylistParseResult::FINISHED
                                   : utils::playlistParser::PlaylistParseResult::STILL_ONGOING};
        }
        auto contentFetcher = m_contentFetcherFactory->create(urlAndInfo.url);
        if (!contentFetcher) {
            AISDK_ERROR(LX("nextFailed").d("reason", "createContentFetcherFailed").sensitive("url", urlAndInfo.url));
            return PlaylistEntry{.url = urlAndInfo.url,
                                 .duration = urlAndInfo.length,
                                 .parseResult = utils::playlistParser::PlaylistParseResult::ERROR};
        }

        auto httpContent = contentFetcher->getContent(
            dmInterface::HTTPContentFetcherInterface::FetchOptions::CONTENT_TYPE);
        if (!httpContent) {
            AISDK_ERROR(LX("getHTTPContent").d("reason", "nullHTTPContentReceived").sensitive("url", urlAndInfo.url));
            return PlaylistEntry{.url = urlAndInfo.url,
                                 .duration = urlAndInfo.length,
                                 .parseResult = utils::playlistParser::PlaylistParseResult::ERROR};
        }
        do {
            if (m_abort) {
                AISDK_DEBUG5(LX("nextFailed").d("info", "aborting"));
                break;
            }
        } while (!httpContent->isReady(WAIT_FOR_FUTURE_READY_TIMEOUT));
        if (!(*httpContent)) {
            AISDK_ERROR(LX("getHTTPContent").d("reason", "badHTTPContentReceived").sensitive("url", urlAndInfo.url));
            return PlaylistEntry{.url = urlAndInfo.url,
                                 .duration = urlAndInfo.length,
                                 .parseResult = utils::playlistParser::PlaylistParseResult::ERROR};
        }
        ...
        AISDK_DEBUG5(LX("IterativePlaylistParser")
        ...
        if (contentType.find(M3U_CONTENT_TYPE) != std::string::npos) {
            ...
            if (!getPlaylistContent(m_contentFetcherFactory->create(urlAndInfo.url), &playlistContent)) {
                AISDK_ERROR(LX("failedToRetrieveContent").sensitive("url", urlAndInfo.url));
                return PlaylistEntry{.url = urlAndInfo.url,
                                     .duration = urlAndInfo.length,
                                     .parseResult = utils::playlistParser::PlaylistParseResult::ERROR};
            }
            ...
            if (isExtendedM3U) {
                AISDK_DEBUG5(LX("isExtendedM3U").sensitive("url", urlAndInfo.url));
            } else {
                AISDK_DEBUG5(LX("isPlainM3UPlaylist").sensitive("url", urlAndInfo.url));
            }
            ...
            if (childrenUrls.empty()) {
                AISDK_ERROR(LX("noChildrenURLs"));
                return PlaylistEntry{.url = urlAndInfo.url,
                                     .duration = urlAndInfo.length,
                                     .parseResult = utils::playlistParser::PlaylistParseResult::ERROR};
            }
            AISDK_DEBUG5((LX("foundChildrenURLsInPlaylist").d("num", childrenUrls.size())));
            if (isExtendedM3U) {
                if (M3UContent.streamInfTagPresent) {
                    ...
                    AISDK_DEBUG5(LX("encounteredVariantStream").sensitive("url", urlAndInfo.url));
            ...
            AISDK_DEBUG5(LX("isPLSPlaylist").sensitive("url", urlAndInfo.url));
            std::string playlistContent;
            if (!getPlaylistContent(m_contentFetcherFactory->create(urlAndInfo.url), &playlistContent)) {
                return PlaylistEntry{.url = urlAndInfo.url,
                                     .duration = urlAndInfo.length,
            ...
            AISDK_DEBUG5(LX("foundNonPlaylistURL"));
    ...
    AISDK_DEBUG0(LX("nextFailed").d("reason", "parseAborted"));
...
bool IterativePlaylistParser::getPlaylistContent(
    ...
    std::string* content) {
    if (!contentFetcher) {
        AISDK_ERROR(LX("getContentFromPlaylistUrlIntoStringFailed").d("reason", "nullContentFetcher"));
        return false;
    }

    auto httpContent =
        contentFetcher->getContent(dmInterface::HTTPContentFetcherInterface::FetchOptions::ENTIRE_BODY);
    if (!httpContent) {
        AISDK_ERROR(LX("getContentFromPlaylistUrlIntoStringFailed").d("reason", "nullHTTPContentReceived"));
        return false;
    }
    do {
        if (m_abort) {
            AISDK_DEBUG5(LX("getContentFromPlaylistUrlIntoStringFailed").d("reason", "abortParsing"));
            return false;
        }
    } while (!httpContent->isReady(WAIT_FOR_FUTURE_READY_TIMEOUT));
