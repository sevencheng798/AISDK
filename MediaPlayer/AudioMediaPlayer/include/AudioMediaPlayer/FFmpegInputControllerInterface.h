...
public:
	...
	/**
	 * This method will only allocate an AVFormatContext that represents the current input stream.
	 *
	 * @return A original pointer to @c AVFormatContext.
	 */
	virtual AVFormatContext* createNewFormatContext() { return nullptr; };
    /**
     * This method will set some options and open an input stream.
     *
     * @return A tuple with the return result, a shared pointer to @c AVFormatContext and the initial playback position.
     */
    virtual std::tuple<Result, std::shared_ptr<AVFormatContext>, std::chrono::milliseconds>
    getCurrentFormatContextOpen() = 0;
