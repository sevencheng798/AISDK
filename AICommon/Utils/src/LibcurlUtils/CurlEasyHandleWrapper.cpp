...
//#include "AICommon/Utils/Configuration/ConfigurationNode.h"
#include <Utils/LibcurlUtils/CurlEasyHandleWrapper.h>
#include <Utils/LibcurlUtils/HttpResponseCodes.h>
//#include <AICommon/Utils/LibcurlUtils/LibcurlUtils.h>
...
CurlEasyHandleWrapper::CurlEasyHandleWrapper() :
        ...
        m_lastPost{nullptr} {
    if (m_handle == nullptr) {
        AISDK_ERROR(LX("CurlEasyHandleWrapperFailed").d("reason", "curl_easy_init failed"));
    } else {
        setDefaultOptions();
    }
};
...
bool CurlEasyHandleWrapper::reset() {
    ...
    if (ret != CURLE_OK) {
        ...
        if (!m_handle) {
            AISDK_ERROR(LX("resetFailed").d("reason", "curlFailure").d("method", "curl_easy_init"));
            return false;
        }
    ...
    if (HTTPResponseCode::SUCCESS_NO_CONTENT == responseCode) {
        AISDK_ERROR(LX("reset").d("responseCode", "HTTP_RESPONSE_SUCCESS_NO_CONTENT"));
        curl_easy_cleanup(m_handle);
        m_handle = curl_easy_init();
        if (!m_handle) {
            AISDK_ERROR(LX("resetFailed").d("reason", "curlFailure").d("method", "curl_easy_init"));
...
bool CurlEasyHandleWrapper::addHTTPHeader(const std::string& header) {
    ...
    if (!m_requestHeaders) {
        AISDK_ERROR(LX("addHTTPHeaderFailed").d("reason", "curlFailure").d("method", "curl_slist_append"));
        AISDK_DEBUG(LX("addHTTPHeaderFailed").sensitive("header", header));
        return false;
    }
...
bool CurlEasyHandleWrapper::addPostHeader(const std::string& header) {
    ...
    if (!m_postHeaders) {
        AISDK_ERROR(LX("addPostHeaderFailed").d("reason", "curlFailure").d("method", "curl_slist_append"));
        AISDK_DEBUG(LX("addPostHeaderFailed").sensitive("header", header));
        return false;
    }
    return true;
}
...
bool CurlEasyHandleWrapper::setPostContent(const std::string& fieldName, const std::string& payload) {
    CURLFORMcode ret = curl_formadd(
        ...
        CURLFORM_END);
    if (ret) {
        AISDK_ERROR(LX("setPostContentFailed")
                        .d("reason", "curlFailure")
...
bool CurlEasyHandleWrapper::addPostStream(const std::string& fieldName, void* userData) {
    CURLFORMcode ret = curl_formadd(
        ...
        CURLFORM_END);
    if (ret) {
        AISDK_ERROR(LX("setPostStreamFailed")
                        .d("reason", "curlFailure")
                        .d("method", "curl_formadd")
                        .d("fieldName", fieldName)
                        .d("curlFormCode", ret));
...
bool CurlEasyHandleWrapper::prepareForTLS1() {
   // if (!m_handle) {
   //     AISDK_ERROR(LX("prepareForTLS1Failed").d("reason", "nullHandle"));
   //     return false;
   // }
    if (!(setopt(CURLOPT_SSLVERSION, CURL_SSLVERSION_TLSv1_2) &&
          setopt(CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_2_0) &&
          setopt(CURLOPT_USE_SSL, CURLUSESSL_ALL) && setopt(CURLOPT_SSL_VERIFYPEER, 1L) &&
	...
	/*
	* To-Do 
	* if support TLS options, specifies a value for the libcurl property CURLOPT_CAPATH
	*/
//	std::string caPath;
//	setopt(handle, CURLOPT_CAPATH, caPath.c_str());
...
bool CurlEasyHandleWrapper::setDefaultOptions() {
    ...
    do {
        if (!prepareForTLS1()) {
            AISDK_ERROR(LX("setDefaultOptions").d("reason", "prepareForTLS failed"));
            break;
        }
