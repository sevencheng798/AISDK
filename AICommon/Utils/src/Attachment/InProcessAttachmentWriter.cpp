 ...
 */
#define LX(event) logging::LogEntry(TAG, event)
...
std::unique_ptr<InProcessAttachmentWriter> InProcessAttachmentWriter::create(
    ...
    if (!writer->m_writer) {
        AISDK_ERROR(LX("createFailed").d("reason", "could not create instance"));
        return nullptr;
    }
...
std::size_t InProcessAttachmentWriter::write(
    ...
    if (!writeStatus) {
        AISDK_ERROR(LX("writeFailed").d("reason", "writeStatus is nullptr"));
        return 0;
    }

    if (!m_writer) {
        AISDK_ERROR(LX("writeFailed").d("reason", "SDS is closed or uninitialized"));
        *writeStatus = WriteStatus::CLOSED;
        AISDK_ERROR(LX("InProcessAttachmentWriter : SDS is closed!"));
        return 0;
    }
    ...
    *writeStatus = WriteStatus::OK;
    ...
    if (numBytes < wordSize) {
        AISDK_ERROR(LX("writeFailed").d("reason", "number of bytes are smaller than the underlying word size"));
    ...
    if (writeResult < 0) {
        switch (writeResult) {
            ...
            case SDSTypeWriter::Error::WOULDBLOCK:
                *writeStatus = WriteStatus::OK_BUFFER_FULL;
                break;

            // This means a parameter passed to the underlying SDS writer was invalid.
            case SDSTypeWriter::Error::INVALID:
                AISDK_ERROR(LX("AttachmentWriter has generated an internal error."));
                *writeStatus = WriteStatus::ERROR_INTERNAL;
                break;
            case SDSTypeWriter::Error::TIMEDOUT:
                AISDK_DEBUG5(LX("AttachmentWriter has timed out while attempting a write."));
        ...
        *writeStatus = WriteStatus::CLOSED;
        AISDK_INFO(LX("writeFailed").d("reason", "underlying SDS is closed"));
        close();
    } else {
